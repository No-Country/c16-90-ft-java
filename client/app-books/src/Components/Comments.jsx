...
const Comments = ({ bookId }) => {
  ...
  // Cargar los comentarios iniciales al montar el componente
  useEffect(() => {
    const existingComments = JSON.parse(localStorage.getItem("comments")) || [];
    const filteredComments = existingComments.filter(
      (comment) => comment.bookId === parseInt(bookId)
    );
          ...
          <div className="px-10 max-w-[1300px] break-words w-full">
            <div className="bg-white w-full rounded-2xl px-10 py-8 shadow-lg hover:shadow-2xl transition duration-500">
              <div className="mt-4">
                ...
                <p className="mt-4 text-md text-gray-600">{e?.comment}</p>
              </div>
            </div>
          </div>
        </div>
