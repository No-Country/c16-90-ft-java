...
const fieldsSignup =[
...
},
  {
    name: 'email', 
    type: 'email', 
    placeholder: 'E-mail', icon: <AiOutlineMail/>, 
    validation: {
      required: 'This is required',
      pattern: {
        value: /^(([^<>()\[\]\.,;:\s@"]+(\.[^<>()\[\]\.,;:\s@"]+)*)|(".+"))@(([^<>()\.,;:\s@"]+\.)+[^<>()\.,;:\s@"]{2,})$/,
      ...
      <Routes>
        <Route path="/" element={<SearchBar />}>
          ...
          <Route path="/userpage" element={<UserPage />}>
            ...
            <Route index element={<FeedReusable section='Read' />} path="read" />
            <Route index element={<FeedReusable section='Favorites'/>} path="favorites" />
            <Route index element={<FeedReusable section='Wish'/>} path="wish" />
          </Route>
        </Route>
